...
<body class="bg-gradient-to-br from-white via-sky-50 to-emerald-100 text-gray-800 min-h-screen">
    <header class="bg-white sticky top-0 z-50 shadow-md">
        <nav class="container max-w-screen-xl mx-auto px-4 py-4 flex flex-wrap sm:flex-nowrap justify-between items-center">
            <a href="{{ route('admin.panel') }}" class="text-xl sm:text-2xl font-bold text-emerald-700 mb-2 sm:mb-0">Teams</a>
    ...
    <main class="max-w-7xl mx-auto px-4 py-10 space-y-6">
        <div class="text-center mb-12">
            <h2 class="text-xl sm:text-2xl font-semibold text-emerald-700">Manage Our Teams!</h2>
        </div>
        <div class="text-right">
            <a href="{{ route('team.create') }}" class="bg-emerald-600 text-white px-4 py-2 sm:px-5 sm:py-3 rounded-lg shadow hover:bg-emerald-800 transition text-sm sm:text-base">+ Add Teams</a>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 justify-items-center">
            @foreach($teams as $team)
            <div class="bg-white rounded-2xl shadow p-5 text-center flex flex-col justify-between h-36 w-full hover:shadow-md hover:bg-green-50">
                <div class="flex-grow flex flex-col justify-center">
                    <h3 class="text-lg font-semibold text-gray-800">{{ $team->name }}</h3>
                    <p class="text-sm text-gray-500">{{ $team->position }}</p>
                </div>
                <div class="mt-4 flex justify-center gap-3">
                    <a href="{{ route('team.edit', $team->id) }}" class="text-yellow-500 hover:text-yellow-800 text-lg sm:text-xl">
